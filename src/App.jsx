...
function App() {
  ...
  const missionSelectedStars = useMemo(
  ...
  const allJudged = missionSelectedStars.length === missionTargetCount
    && missionSelectedStars.length > 0
    && missionSelectedStars.every((star) => star.userClassification);
  ...
  function generateReport() {
    const targets = missionSelectedStars.length ? missionSelectedStars : simState.stars.filter((star) => star.observed).slice(0, 3);
  ...
  useEffect(() => {
    if (taskMode && stage === 'report' && missionSelectedStars.length >= 1 && !latestReport?.title.includes('15分钟科研探索报告')) {
      generateReport();
    }
  }, [taskMode, stage, missionSelectedStars.length, latestReport?.title]);
